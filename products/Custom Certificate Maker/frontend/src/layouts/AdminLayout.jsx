...
export const AdminLayout = () => {
  ...
  const mgmtNav = [
    ...
    { path: '/admin/notifications', label: 'Notifications', icon: Bell, badge: '4' },
  ...
  const getPageTitle = () => {
    const current = [...mainNav, ...mgmtNav, ...systemNav].find((n) => n.path === location.pathname);
    return current ? current.label : 'Super Admin Control Center';
  };
      ...
      <header className="h-16 bg-white border-b border-slate-200 sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 shadow-sm">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="hidden md:flex items-center relative w-64">
            ...
            <input
              type="text"
              placeholder="Search certificates, orgs, recipients..."
          ...
          <Link
            to="/org/designer"
            className="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs shadow-md shadow-orange-500/20 transition-all hover:scale-[1.02]"
          >
            ...
            {notifDropdownOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-2xl shadow-2xl p-4 z-50 space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  ...
                  <span className="text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full">4 New</span>
                </div>
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  <div className="p-2 rounded-xl bg-blue-50/60 text-xs space-y-0.5 border border-blue-100">
                    <p className="font-bold text-blue-900">New Organization Registered</p>
                    <p className="text-[11px] text-slate-600">Tech Academy created an issuer profile.</p>
                    <p className="text-[9px] text-slate-400 font-mono">10 mins ago</p>
                  </div>
                  <div className="p-2 rounded-xl bg-slate-50 text-xs space-y-0.5">
                    <p className="font-bold text-slate-800">120 Certificates Issued</p>
                    <p className="text-[11px] text-slate-500">High volume reported today.</p>
                  </div>
                </div>
                <Link
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="text-left hidden lg:block">
                ...
                <p className="text-[10px] font-medium text-slate-500 mt-0.5">Master Control</p>
              </div>
          ...
          <div className="h-16 flex items-center px-5 border-b border-slate-100 space-x-3">
            ...
            {sidebarOpen && (
              <div>
                ...
                <p className="text-[9px] font-semibold text-blue-600 uppercase tracking-widest mt-0.5">
                  Super Admin SaaS
                </p>
              </div>
            )}
            ...
            <div className="flex items-center space-x-3 p-2 rounded-xl bg-white border border-slate-200 shadow-sm">
              ...
              {sidebarOpen && (
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] font-semibold text-orange-600 truncate">Platform Administrator</p>
                </div>
              )}
            </div>
          </div>
