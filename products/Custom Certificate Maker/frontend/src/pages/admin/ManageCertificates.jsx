...
import { Award, Search, Eye, Download, RotateCcw, XCircle, CheckCircle, ExternalLink, Filter } from 'lucide-react';
...
export const ManageCertificates = () => {
  ...
  const handleRevoke = async (id) => {
    ...
    try {
      const res = await api.revokeCertificate(id, 'Revoked by Super Admin');
      if (res.success) {
  ...
  const handleRestore = async (id) => {
    try {
      const res = await api.restoreCertificate(id);
      if (res.success) {
        alert('Certificate restored to Valid status.');
        fetchCertificates();
      }
    } catch (err) {
  ...
  if (loading) return <LoadingSpinner label="Loading certificate master records..." />;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Manage All Certificates</h1>
          <p className="text-xs text-slate-500 mt-1 font-medium">
            Search, inspect, verify, revoke, or restore any issued credential across the platform.
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-900">
            Certificate Master Records ({filteredCerts.length})
          </h3>
              ...
              {filteredCerts.length > 0 ? (
                    ...
                    <td className="py-4 px-6 text-right space-x-2">
                      ...
                      {cert.status === 'Revoked' ? (
                        <button
                          ...
                          className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 font-bold rounded-xl text-xs transition-colors"
                        >
                          Restore
                        </button>
                      ) : (
                ...
                <tr>
                  <td colSpan="6" className="py-8 text-center text-slate-400 text-xs">
                    No certificates matched your query.
                  </td>
                </tr>
              )}
            </tbody>
