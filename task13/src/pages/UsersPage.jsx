...
export default function UsersPage({
  ...
  const loadUsers = async (isManualRefresh = false) => {
    ...
    try {
      ...
      if (error && !isMock) {
        ...
        if (isManualRefresh) {
          showToast(`Synced ${data?.length || 0} user records!`, 'success')
        }
      }
    } catch (err) {
  ...
  const availableCountries = useMemo(() => {
    ...
    users.forEach(u => { if (u.country) countries.add(u.country) })
  ...
  const handleSelectAll = (checked) => {
    if (checked) setSelectedIds(filteredAndSortedUsers.map(u => u.id))
    else setSelectedIds([])
  }
